...
export default function Users() {
              ...
              <Popover id={`popover-actions-${row.id}`} className="popoverdropdown">
                ...
                >
                  <div className="d-flex flex-column">
                    <Button variant="link" as={Link} className="dropdownitem" to="/users/viewuser"><Icon icon="solar:eye-linear" width={16} height={16} className="me-1" />View</Button>
                    <Button variant="link" as={Link} className="dropdownitem" to="/users/edituser"><Icon icon="mynaui:edit" width={16} height={16} className="me-1" />Edit</Button>
  ...
  const loadUsers = useCallback(
    async ({ limit = DEFAULT_LIMIT, search = "" } = {}) => {
      if (!getAccessToken()) {
        return;
      }
      ...
      try {
        ...
        const mapped = users.map((user, index) => ({
          id: user.id ?? index,
          img: resolveProfileImage(user.profile),
          name: user.name || "",
          Email: user.email || "",
          Phone: user.phone_no || user.phone_number || user.phone || "",
          Date: (() => {
  ...
  useEffect(() => {
    const handler = setTimeout(() => {
      loadUsers({ limit: DEFAULT_LIMIT, search: searchText });
    }, 300);
  ...
  const filteredData = useMemo(() => {
    ...
    return result;
  }, [usersData, statusFilter, searchText]);

  return (
    <React.Fragment>
      <div className="d-flex flex-wrap align-items-center justify-content-between mb_20">
        <h4 className="mainheading">Users</h4>
        <Button as={Link} className="btn-sm" to='/users/adduser'><Icon icon="ic:twotone-plus" width={22} height={22} />Add Users</Button>
      </div>
